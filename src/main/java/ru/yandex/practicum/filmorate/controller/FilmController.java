...
public class FilmController {
    ...
    public Film getFilmById(@PathVariable Integer id) {
        log.info(String.format("Пришёл запрос на получение фильма с id = %d", id));
    ...
    public Collection<Film> getDirectorFilms(@PathVariable Integer directorId
        ...
        log.info(String.format("Пришёл запрос на получение режиссера с id = %d", directorId));
    ...
    public Collection<Film> getMostPopularFilms(@RequestParam(required = false, defaultValue = "10") Integer count,
        ...
        log.info(String.format("Пришёл запрос на получение популярных фильмов. count = %d, genreId = %d, year = %d",
                count, genreId, year));
    ...
    public Collection<Film> search(@RequestParam String query,
        ...
        log.info(String.format("Пришёл запрос на поиск фильмов. query = %s, by = %s", query, by));
